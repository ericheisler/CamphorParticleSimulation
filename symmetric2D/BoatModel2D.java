...
class BoatModel2D {
	...
	public double R, W, m, vis, L, rc, D, volD, k, volk, a, vola, beta, csat, maxdgamma;
	...
	public ArrayList<Boat2D> b;
	public double[][] c; // surface c
	public double[][] volc; // volume c
	...
	// for the ADI method for volume c
	double vupper, vlower, vdiag;
	double vtmpm, vtmpdiag;
	double vlambda;
	double vusefulnumber, vadt;
	double [][]vtmpc;
	double [] vrhs;
	double [][] vfn, vfnh, vfnp;
	...
	public BoatModel2D(){
		...
		b = new ArrayList<Boat2D>();
		c = new double[nx][ny];
		volc = new double[nx][ny];
		...
		vtmpc = new double[nx][ny];
		vrhs = new double[nx];
		vfn = new double[nx][ny];
		vfnh = new double[nx][ny];
		vfnp = new double[nx][ny];
		...
		vlambda = volD*dt/2/dx/dx;
		vusefulnumber = 1-2*vlambda - dt*volk/4;
		vadt = dt/4;
		vdiag = 1.0+2*vlambda + dt*volk/4;
		vupper = -vlambda;
		vlower = -vlambda;
		vtmpm = vlower/vdiag;
		vtmpdiag = vdiag - vtmpm*vupper;
	...
	// it is elastic
	public void boatCollision(Boat2D b1, Boat2D b2){
		// distance
	...
	public double[][] setUpF(double[][] ff, double[][] vff){
		// clear the vector
		for (int i=0; i<nx; i++){
			for (int j=0; j<ny; j++){
				ff[i][j] = 0.0;
			...
			if(left < right){
				for(int xin=left; xin<=right; xin++){
					for(int yin=bottom; yin<=top; yin++){
						...
						if(distsq > (rc-dx)*(rc-dx) && distsq < (rc+dx)*(rc+dx)){
							ff[xin][yin] = a*(1.0-(c[xin][yin]/csat)*(c[xin][yin]/csat)*(c[xin][yin]/csat));
						}
						if(distsq < rc*rc){
							vff[xin][yin] = vola*(1.0-(volc[xin][yin]/csat)*(volc[xin][yin]/csat)*(volc[xin][yin]/csat));
						}
						if(distsq < L*L/4){
	...
	public void transferc(int whichOne){
		// just add it onto the f parts
		double vkdifference = 0.0;
		if(whichOne == 0){
			for (int i=0; i<nx; i++){
				for (int j=0; j<ny; j++){
					vkdifference = volk*(c[i][j] - volc[i][j]);
					
					fnh[i][j] -= vkdifference;
					fnp[i][j] -= vkdifference;
					
					vfnh[i][j] += vkdifference;
					vfnp[i][j] += vkdifference;
				}
			}
		}else if(whichOne == 1){
			for (int i=0; i<nx; i++){
		...
		if(direction == 1){
			...
			*/
			...
			for (int i=1; i<ny-1; i++){
				//tmpm = lower/diag;
				//tmpdiag = diag - tmpm*upper;
				vrhs[i] = vrhs[i] - vtmpm*vrhs[i-1];
			}
			vrhs[ny-1] = vrhs[ny-1] - 2*vtmpm*vrhs[ny-2]; // this is for the boundary
			
			vtmpc[index][ny-1] = vrhs[ny-1]/(vdiag - 2*vtmpm*vupper); // this is for the boundary
			for (int i=ny-2; i>1; i--){
				vtmpc[index][i] = (vrhs[i]-vupper*vtmpc[index][i+1])/vtmpdiag;
			}
			vtmpc[index][1] = (vrhs[1]-vupper*vtmpc[index][2])/(vdiag - vtmpm*2*vupper); // this is for the boundary
			vtmpc[index][0] = (vrhs[0]-2*vupper*vtmpc[index][1])/vdiag; // this is for the boundary
		}
	}
	...
	public void solveXDirection(int index){
		...
		//find the products
		double tvy = y[0]-y[nx-1]*upper;
		double tvq = q[0]-q[nx-1]*upper;
		
		//finally solve for c
		for (int i = 0; i < nx; i++){
			tmpc[i][index] = y[i] - (tvy/(1+tvq))*q[i];
        }
		
		// and for volume////////////////////////////////////////////////////
		id = 0.0;
		// solve for y
		vmc[0] = vupper/(vdiag+1);
		vmd[0] = vrhs[0]/(vdiag+1);
		for (int i = 1; i < nx-1; i++){
			id = 1/(vdiag - vmc[i-1]*vlower);
			vmc[i] = vupper*id;
			vmd[i] = (vrhs[i] - vmd[i-1]*vlower)*id;
        }
		id = 1/(vdiag + vupper*vlower - vmc[nx-2]*vlower);
		vmc[nx-1] = vupper*id;
		vmd[nx-1] = (vrhs[nx-1] - vmd[nx-2]*vlower)*id;
		...
		for (int i = 1; i < nx-1; i++){
			id = 1/(vdiag - vmc[i-1]*vlower);
			vmc[i] = vupper*id;
			vmd[i] = -vmd[i-1]*vlower*id;
        }
		id = 1/(vdiag + vupper*vlower - vmc[nx-2]*vlower);
		vmc[nx-1] = vupper*id;
		vmd[nx-1] = (vlower - vmd[nx-2]*vlower)*id;
		...
		//find the products
		tvy = vy[0]-vy[nx-1]*vupper;
		tvq = vq[0]-vq[nx-1]*vupper;
		
		//finally solve for c
		for (int i = 0; i < nx; i++){
			vtmpc[i][index] = vy[i] - (tvy/(1+tvq))*vq[i];
        }
	}
	...
	public void timeStep(){
		...
		// include the transfer between surf and vol
		transferc(0);
	...
	public void fullStepADI(){
		...
		for(int yind=1; yind<ny-1; yind++){
			...
			for (int i=0; i<nx; i++){
				...
				vrhs[i] = vlambda*(volc[i][yind-1]+volc[i][yind+1]) + vusefulnumber*volc[i][yind] + vadt*(vfn[i][yind]+vfnh[i][yind]);
			}
		...
		for (int i=0; i<nx; i++){
			...
			vrhs[i] = vlambda*(2*volc[i][1]) + vusefulnumber*volc[i][0] + vadt*(vfn[i][0]+vfnh[i][0]);
		}
		solveSystem(1, 0);
		for (int i=0; i<nx; i++){
			rhs[i] = lambda*(2*c[i][ny-2]) + usefulnumber*c[i][ny-1] + adt*(fn[i][ny-1]+fnh[i][ny-1]);
			vrhs[i] = vlambda*(2*volc[i][ny-2]) + vusefulnumber*volc[i][ny-1] + vadt*(vfn[i][ny-1]+vfnh[i][ny-1]);
		}
		solveSystem(1, ny-1);
		...
		for(int xind=1; xind<nx-1; xind++){
			...
			for (int i=0; i<ny; i++){
				...
				vrhs[i] = vlambda*(volc[xind-1][i]+volc[xind+1][i]) + vusefulnumber*volc[xind][i] + vadt*(vfnh[xind][i]+vfnp[xind][i]);
			}
			//B is already set, so solve the system for c
			solveSystem(-1, xind);
		}
		...
		for (int i=0; i<ny; i++){
			...
			vrhs[i] = vlambda*(volc[1][i] + volc[nx-1][i]) + vusefulnumber*volc[0][i] + vadt*(vfnh[0][i]+vfnp[0][i]);
		}
		solveSystem(-1, 0);
		for (int i=0; i<ny; i++){
			rhs[i] = lambda*(c[nx-2][i] + c[0][i]) + usefulnumber*c[nx-1][i] + adt*(fnh[nx-1][i]+fnp[nx-1][i]);
			vrhs[i] = vlambda*(volc[nx-2][i] + volc[0][i]) + vusefulnumber*volc[nx-1][i] + vadt*(vfnh[nx-1][i]+vfnp[nx-1][i]);
		}
		solveSystem(-1, nx-1);
	...
	public ArrayList<Boat2D> getBoats(){
		return b;
	}
