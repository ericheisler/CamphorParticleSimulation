...
public class Plot2D extends BufferedImage {
	...
	Line2D.Double[] extras;
	Color[] extrasColors;

	boolean boxed, showAxes, xlimSet, ylimSet, lines, drawExtras;
	...
	public void addLine(double x1, double y1, double x2, double y2, Color col){
		if(extras == null){
			extras = new Line2D.Double[1];
			extras[0] = new Line2D.Double(x1, y1, x2, y2);
			extrasColors = new Color[1];
			extrasColors[0] = col;
		}else{
			Line2D.Double[] temp = new Line2D.Double[extras.length+1];
			Color[] temp2 = new Color[extras.length+1];
			for(int i=0; i<extras.length; i++){
				temp[i] = extras[i];
				temp2[i] = extrasColors[i];
			}
			temp[extras.length] = new Line2D.Double(x1, y1, x2, y2);
			temp2[extras.length] = col;
			extras = temp;
			extrasColors = temp2;
		}
		drawExtras = true;
	...
	public void paint(){
		...
		// and finally finally, draw the extras
		if(drawExtras && extras != null){
			int x1, y1, x2, y2;
			for(int i=0; i<extras.length; i++){
				x1 = (int)((extras[i].getX1()-xmin)/(xmax-xmin)*pwidth+yPad);
				x2 = (int)((extras[i].getX2()-xmin)/(xmax-xmin)*pwidth+yPad);
				y1 = (int)(topPad+pheight - (extras[i].getY1()-ymin)/(ymax-ymin)*pheight);
				y2 = (int)(topPad+pheight - (extras[i].getY2()-ymin)/(ymax-ymin)*pheight);
				g.setColor(extrasColors[i]);
